...
class UKF : public KF {
 ...
 private:
  ...
  void predict_sigma_points_mean_cov(ImuData::ConstPtr last_imu, ImuData::ConstPtr curr_imu) {
    ...
    Eigen::Quaterniond q = Eigen::Quaterniond(state_ptr_->Rwb_);
    OsgManager::getInstance()->updatePosition(q.x(), q.y(), q.z(), q.w(), state_ptr_->p_wb_[0], state_ptr_->p_wb_[1], state_ptr_->p_wb_[2]);
  }
